...
export function useTTS() {
  ...
  const speak = useCallback((text, options = {}) => {
    ...
    utterance.onerror = (event) => {
      ...
      // before a new utterance — not real errors
  ...
  }, [isSupported, voices, defaultVoice]);
  ...
  return useMemo(() => ({
    ...
    voices,
    defaultVoice,
  }), [speak, cancel, pause, resume, isSupported, isSpeaking, voices, defaultVoice]);
}
...
export function announceTeamServed(team, field, tts, options = {}) {
  ...
  // Cancel any previous speech, then queue both announcements
  // Using cancel() + queue:true avoids the race condition where
  // queue:false would cancel() and speak() in the same call
  tts.cancel();
