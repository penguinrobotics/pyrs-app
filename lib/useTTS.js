...
export function announceTeamServed(team, field, tts, options = {}) {
  ...
  // Delay speaking to let cancel() fully resolve — some browsers
  // will interrupt newly queued utterances if speak() is called
  ...
  setTimeout(() => {
    tts.speak(text, {
      ...
      voice: options.voice,
      queue: true, // Already canceled above, just queue
    });
