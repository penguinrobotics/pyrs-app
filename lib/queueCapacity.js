...
function calculateQueueStatus(settings, queueState, currentTime = null) {
  ...
  const pacificNow = currentTime || getCurrentPacificTime();
  ...
  // Parse cutoff date/time (interpreted as Pacific time)
  const cutoff = parseDateTime(skillsCutoffTime);
  if (!cutoff) {
    return { isOpen: true, reason: 'invalid_cutoff' };
  }

  // Build cutoff date using current year (in Pacific time context)
  ...
  const minutesRemaining = Math.floor((cutoffDate - pacificNow) / (1000 * 60));
...
module.exports = { calculateQueueStatus, parseDateTime, getCurrentPacificTime };
